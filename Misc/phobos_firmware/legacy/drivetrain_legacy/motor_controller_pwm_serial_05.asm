...
;and 1-reverse, bits 0-6 gives you ticks per 0.1s. That gives you a 
limit of
;127 ticks/.1s
;---------------------------------------------------------------------
...
;code to setup the timers for PWM, only do this part once
	...
	;intialize the variable DIR for direction to 0
	;0 denotes forwardand 1 denotes reverse
	LDAA #$00
	STAA DIR,X
...
SkipReadSCI:			;nothing to read do nothing
	...
	ORAA DIR			;OR A with DIR, this will give the number a sign
...
HERE2
	...
	;now we have the numer 10 or 08 in A which tells us direction
	;we need to figure out which one and set the DIR variable high 
	;or low

	;compare A with 000010000  $08
	;if it is successful the zero flag will go high
	;then check the zero flag and if its high
	;set the DIR var. high

	CMPA #$08	;compare contents of A to 00001000					2		
	BEQ SET2	;if compare is successful, go to set3					3
...
OVER2	
	...
	LDAA #$00
	STAA PORTA,X	;	clear portA						COUNTS --4
