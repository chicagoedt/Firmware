...
/**
 * @brief Stores the command sent by the transmitter.
 *
 * This variable is TRUE if the wireless transmitter is sending a stop signal
 * or FALSE otherwise.
 *
 * This variable is volatile because it is modified by interrupts.
 */
static volatile BOOL_T rfStop;

/**
 * @brief Stores whether the connection to the transmitter has been lost.
 *
 * This variable is TRUE if no signal has been received from the transmitter
 * for longer than the timeout period or FALSE otherwise.
 * or FALSE otherwise.
 *
 * This variable is volatile because it is modified by interrupts.
 */
static volatile BOOL_T lostSignal;
 ...
 * @author Anup Kotadia anup.kotadia@gmail.com
 */
static void providePower(BOOL_T givePower);
 ...
 * @author Anup Kotadia anup.kotadia@gmail.com
 */
static void setConnectionLight(BOOL_T connectionLost, BOOL_T rfStop);
...
ISR(TIMER2_COMP_vect)
{
	...
	providePower(!(isSwitchDown() || rfStop));
	setConnectionLight(lostSignal, rfStop);
}
